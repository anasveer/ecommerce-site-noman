...
const Navbar = () => {
  ...
  // Inside handleSearch function of Navbar.tsx
const handleSearch = (e: React.FormEvent) => {
  e.preventDefault();
  if (query.trim()) {
    setIsMenuOpen(false); // Mobile menu close karein
  ...
  return (
    <nav className="bg-[#0a0f1d] text-white sticky top-0 z-50 border-b border-gray-800 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-1 lg:space-x-4">
            <a
              href="#"
              className="text-gray-300 hover:text-[#d4af37] px-3 py-2 rounded-md text-sm lg:text-base font-medium transition-all duration-200 hover:bg-white/5"
            >
              Home
            </a>
                ...
                <div className="p-2">
                  {categories.map((cat) => (
                    <a
                      key={cat.name}
                      href={cat.href}
                      className="flex items-center gap-3 px-4 py-3 text-sm text-gray-300 hover:text-[#d4af37] hover:bg-white/5 rounded-lg transition-colors"
                    >
                      <span className="text-[#d4af37] bg-[#d4af37]/10 p-2 rounded-lg">
                        {cat.icon}
                      </span>
      ...
      <button type="submit" className="absolute right-3 top-2 text-gray-500">
        <Search className="w-4 h-4" />
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <button className="p-2 text-gray-300 hover:text-[#d4af37] transition-colors relative">
              <ShoppingCart className="h-6 w-6" />
              <span className="absolute top-1 right-1 bg-[#d4af37] text-[#0a0f1d] text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                0
              </span>
            </button>
      ...
      <div
        className={`fixed inset-0 z-[60] md:hidden transition-transform duration-500 ease-in-out ${
      ...
      <button type="submit" className="absolute right-3 top-2 text-gray-500">
        <Search className="w-4 h-4" />
            ...
            <a
              href="#"
              className="block px-4 py-4 text-lg font-medium text-gray-200 hover:bg-[#d4af37]/5 hover:text-[#d4af37] rounded-xl transition-all"
            >
              Home
            </a>
            ...
            <div className="space-y-1">
              ...
              <div
                className={`overflow-hidden transition-all duration-300 ${
                  isCategoryOpen ? 'max-h-48' : 'max-h-0'
                }`}
              >
                {categories.map((cat) => (
                  <a
                    key={cat.name}
                    href={cat.href}
                    className="flex items-center gap-4 px-8 py-3 text-gray-400 hover:text-[#d4af37] transition-colors"
                  >
